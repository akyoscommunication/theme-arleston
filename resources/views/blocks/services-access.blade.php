<section class="s-services {{ $classes }}" style="{{ $styles }}">
  <div class="s-services__background {{ $classes }}"></div>
  <div class="container">
    ...
    <div class="s-services-list">
      @if(count($services) <= 4)
        @php
            $maxRowsExists = array_reduce($services, function($carry, $service) {
                $rows = 0;
                ...
                return max($carry, $rows);
            }, 0);
        @endphp
        <div class="s-services-list-wrapper s-services-list-wrapper--peek" style="--grid-rows:{{ $maxRowsExists }}">
